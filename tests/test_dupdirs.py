...
class TestFingerprintDirectory:
    def test_basic_fingerprint(self):
        dir_info = DirInfo(
            path=Path("/test"), file_count=2, total_size=300
        )
        files = [
    ...
    def test_same_content_same_hash(self):
        dir1 = DirInfo(
            path=Path("/a"), file_count=2, total_size=300
        )
        dir2 = DirInfo(
            path=Path("/b"), file_count=2, total_size=300
        )
        files1 = [
            _fi("/a/x.txt", 100, "h1"),
            _fi("/a/y.txt", 200, "h2"),
    ...
    def test_different_content_different_hash(self):
        dir1 = DirInfo(
            path=Path("/a"), file_count=1, total_size=100
        )
        dir2 = DirInfo(
            path=Path("/b"), file_count=1, total_size=200
        )
...
class TestFormatDuplicateDirs:
    ...
    def test_with_groups(self):
        ...
        groups = [
            DuplicateDirGroup(
                group_id=0, fingerprint=fp, directories=[fp, fp]
            )
        ]
