...
from dupeclean.models import DirInfo, DuplicateGroup, FileInfo, ScanStats
...
def analysis_result(tmp_path):
    ...
    dirs = {
        tmp_path: DirInfo(
            path=tmp_path, total_size=sum(f.size for f in files), file_count=len(files)
        )
    }
    stats = ScanStats(
    ...
    dup_group = DuplicateGroup(
        group_id=0, hash_value="abc123", file_size=5, files=[files[0], files[1]]
    )
    return AnalysisResult(
        root=tmp_path, files=files, dirs=dirs, stats=stats, duplicates=[dup_group]
    )
