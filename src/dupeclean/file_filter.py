"""File deduplication file filter for DupeClean.

Advanced file filtering with composable filters.
"""
...
def by_min_size(size: int) -> FileFilter:
    ...
    return FileFilter(f"min_size={size}", lambda fi: fi.size >= size)


def by_max_size(size: int) -> FileFilter:
    """Filter files smaller than size."""
    return FileFilter(f"max_size={size}", lambda fi: fi.size <= size)
...
def by_min_age(days: float) -> FileFilter:
    ...
    return FileFilter(f"min_age={days}d", lambda fi: fi.mtime < cutoff)
...
def by_max_age(days: float) -> FileFilter:
    ...
    cutoff = time.time() - (days * 86400)
    return FileFilter(f"max_age={days}d", lambda fi: fi.mtime >= cutoff)
...
def format_filter_result(original: int, filtered: int, name: str) -> str:
