...
def filter_by_min_count(
    groups: list[DuplicateGroup], min_count: int
) -> list[DuplicateGroup]:
...
def filter_by_min_size(
    groups: list[DuplicateGroup], min_size: int
) -> list[DuplicateGroup]:
    """Filter groups with file size >= min_size."""
    return [g for g in groups if g.file_size >= min_size]


def filter_by_max_size(
    groups: list[DuplicateGroup], max_size: int
) -> list[DuplicateGroup]:
...
def filter_by_extension(
    groups: list[DuplicateGroup], ext: str
) -> list[DuplicateGroup]:
    """Filter groups by file extension."""
    ext_lower = ext.lower().lstrip(".")
    return [
        g for g in groups
        if g.files and g.files[0].ext.lstrip(".").lower() == ext_lower
    ]


def filter_by_min_waste(
    groups: list[DuplicateGroup], min_waste: int
) -> list[DuplicateGroup]:
...
def filter_by_pattern(
    groups: list[DuplicateGroup], pattern: str
) -> list[DuplicateGroup]:
    """Filter groups containing files matching a name pattern."""
    import fnmatch
    return [
        g for g in groups
        if any(fnmatch.fnmatch(f.path.name, pattern) for f in g.files)
    ]
...
def format_filter_result(
    original: int, filtered: int, filter_name: str
) -> str:
